...
class TestWeatherStation:
    ...
    def test_forecast_display(self, capsys):
        ...
        assert "Forecast: Improving weather on the way!" in lines[1]  # Pressure increased
        assert "Forecast: Watch out for cooler, rainy weather" in lines[2]  # Pressure decreased
...
class TestEmailService:
    ...
    def test_confirmation_email_on_email_update(self, capsys):
        ...
        manager.emit("user_email_updated", {
            "user_id": 1,
        ...
        captured = capsys.readouterr()
        assert "Sending confirmation email to alice.new@example.com for user 1" in captured.out
...
class TestAuditService:
    ...
    def test_audit_email_update(self, capsys):
        ...
        captured = capsys.readouterr()
        assert "AUDIT: User 1 changed email from alice@example.com to alice.new@example.com at" in captured.out
        assert len(audit_service.audit_log) == 1
        assert "User 1 changed email from alice@example.com to alice.new@example.com at" in audit_service.audit_log[0]
