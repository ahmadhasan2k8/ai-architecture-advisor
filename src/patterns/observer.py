...
class StatisticsDisplay(WeatherObserver):
    ...
    def update(self, temperature: float, humidity: float, pressure: float) -> None:
        ...
        print(f"Statistics: Avg temp: {avg_temp:.1f}°C, Min: {min_temp}°C, Max: {max_temp}°C")
...
class EmailService:
    ...
    def _setup_listeners(self) -> None:
        ...
        self.event_manager.subscribe("user_email_updated", self._send_email_confirmation)
    ...
    def _send_welcome_email(self, event: Event) -> None:
        ...
        print(f"📧 Sending welcome email to {user_data['name']} at {user_data['email']}")
    ...
    def _send_email_confirmation(self, event: Event) -> None:
        ...
        """
        data = event.data
        print(f"📧 Sending confirmation email to {data['new_email']} for user {data['user_id']}")
