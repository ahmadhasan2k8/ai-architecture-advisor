...
from .code_analyzer import OpportunityType, PatternOpportunity, analyze_directory
...
class RepositoryAnalyzer:
    ...
    def analyze(
        ...
        self.opportunities_by_file = analyze_directory(
            self.repository_path, exclude_patterns=exclude_patterns
        )
    ...
    def _analyze_singleton_usage(
        self, singleton_opportunities: List[PatternOpportunity]
    ) -> None:
        ...
        anti_patterns = [
            opp
            for opp in singleton_opportunities
        ...
        if len(anti_patterns) >= 2:
            self.architectural_insights.append(
        ...
        if len(valid_opportunities) >= 2:
            ...
            db_files = [
                ...
                for opp in valid_opportunities
                if any(
                    keyword in opp.file_path.lower()
    ...
    def _analyze_factory_opportunities(
        self, factory_opportunities: List[PatternOpportunity]
    ) -> None:
    ...
    def _analyze_observer_opportunities(
        self, observer_opportunities: List[PatternOpportunity]
    ) -> None:
    ...
    def _analyze_strategy_opportunities(
        self, strategy_opportunities: List[PatternOpportunity]
    ) -> None:
    ...
    def _analyze_builder_opportunities(
        self, builder_opportunities: List[PatternOpportunity]
    ) -> None:
    ...
    def _analyze_repository_opportunities(
        self, repo_opportunities: List[PatternOpportunity]
    ) -> None:
    ...
    def _analyze_cross_pattern_opportunities(
        self, by_pattern: Dict[str, List[PatternOpportunity]]
    ) -> None:
            ...
            if len(by_pattern["factory"]) >= 1 and len(by_pattern["strategy"]) >= 1:
                self.architectural_insights.append(
                    ArchitecturalInsight(
                        ...
                        affected_files=list(
                            set(
                                [opp.file_path for opp in by_pattern["factory"]]
            ...
            if len(by_pattern["observer"]) >= 1 and len(by_pattern["command"]) >= 1:
                self.architectural_insights.append(
                    ArchitecturalInsight(
                        ...
                        affected_files=list(
                            set(
                                [opp.file_path for opp in by_pattern["observer"]]
    ...
    def _detect_architectural_anti_patterns(
        self, all_opportunities: List[PatternOpportunity]
    ) -> None:
        ...
        high_pattern_usage = {
            pattern: count for pattern, count in pattern_counts.items() if count >= 5
        }

        if high_pattern_usage:
            patterns_list = ", ".join(
                f"{pattern} ({count})" for pattern, count in high_pattern_usage.items()
            )
            self.architectural_insights.append(
                ArchitecturalInsight(
                    insight_type="anti_pattern",
    ...
    def _assess_overall_complexity(
        ...
        high_confidence = len(
            [
                ...
                for opp in all_opportunities
                if opp.confidence
                in [PatternConfidence.HIGH, PatternConfidence.CRITICAL]
            ]
        )
        ...
        if anti_patterns > 0:
            ...
            primary_recommendation = (
                "Implement high-confidence patterns for immediate benefits"
            )
        elif total_opportunities >= 10:
            complexity_level = "Medium (Multiple opportunities available)"
            primary_recommendation = (
                "Prioritize patterns by business value and technical debt reduction"
            )
        elif total_opportunities >= 5:
            complexity_level = "Low-Medium (Some improvement opportunities)"
            primary_recommendation = (
                "Selective pattern implementation based on team capacity"
            )
        else:
    ...
    def _create_analysis_summary(self) -> RepositoryAnalysis:
        ...
        # Top recommendations
        top_insights = sorted(
            self.architectural_insights, key=lambda x: x.priority_score, reverse=True
        )[:5]

        recommendations_summary = []
        for insight in top_insights:
            recommendations_summary.extend(
        ...
        return RepositoryAnalysis(
            ...
            complexity_assessment=next(
                (
                    insight.description
                    for insight in self.architectural_insights
                    if insight.title == "Codebase Complexity Assessment"
                ),
                "Unknown",
            ),
            recommendations_summary=unique_recommendations[
                :10
            ],  # Top 10 recommendations
        )

    def save_analysis(
        self, analysis: RepositoryAnalysis, output_path: Union[str, Path]
    ) -> None:
    ...
    def generate_report(self, analysis: RepositoryAnalysis) -> str:
        ...
        for pattern, count in sorted(
            analysis.pattern_usage_summary.items(), key=lambda x: x[1], reverse=True
        ):
        ...
        high_priority = sorted(
            [opp for opp in all_opportunities if opp.priority_score > 0.7],
            key=lambda x: x.priority_score,
            reverse=True,
        )[:8]
        ...
        for opp in high_priority:
            ...
            report += (
                f"- Effort: {opp.effort_estimate} | Impact: {opp.impact_estimate}\n\n"
            )
